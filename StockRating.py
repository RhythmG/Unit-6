...
def Step3(): #VaR Rating
    ...
    loglower = (sum(sims)/len(sims)) + sqrt(((stdchange)**(2)/2) - lower * sqrt((stdchange)**(2)/trials) + ((stdchange)**(4)/(2*(trials-1))))
    logupper = (sum(sims)/len(sims)) + sqrt(((stdchange)**(2)/2) + upper * sqrt((stdchange)**(2)/trials) + ((stdchange)**(4)/(2*(trials-1))))
